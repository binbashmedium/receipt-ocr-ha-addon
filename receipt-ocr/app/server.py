...
def parse_receipt(lines):
    ...
    def clean_name(s):
        s = s.replace("  ", " ").strip(" .,-")
        return re.sub(r"^[ABC]\b|\b[ABC]$", "", s).strip()
    ...
    for t in lines:
        ...
        if m_price:
            ...
            name_part = clean_name(t[:m_price.start()].strip() or buffer_name)
            buffer_name = ""
            items.append({"qty": 1.0, "name": name_part, "price": price})
            last_item = items[-1]
            continue
...
def process_ocr(image_path, image_name, engine_name="doctr", mqtt_client=None):
    try:
        ...
        entry = {"timestamp": datetime.datetime.now().isoformat(timespec='seconds'),
                 "file": image_name, "engine": engine_name, **parsed}
        ...
        app.logger.info(f"OCR fertig: {image_name}")
...
def start_mqtt_listener():
    ...
    def on_connect(c, userdata, flags, rc):
        app.logger.info(f"[MQTT] Verbunden mit {MQTT_HOST}:{MQTT_PORT}")
        c.subscribe(f"{MQTT_TOPIC}/input")

    def on_message(c, userdata, msg):
        try:
            payload = json.loads(msg.payload.decode())
            filename = payload.get("filename", f"mqtt_{datetime.datetime.now().timestamp()}.jpg")
            data_b64 = payload.get("image_base64")
            if not data_b64:
                app.logger.warning("[MQTT] Kein Bild enthalten.")
                return
            image_data = base64.b64decode(data_b64)
            image_path = os.path.join(DEBUG_DIR, filename)
            with open(image_path, "wb") as f:
                f.write(image_data)
            threading.Thread(target=process_ocr, args=(image_path, filename, "doctr", c), daemon=True).start()
        except Exception as e:
            app.logger.error(f"[MQTT] Fehler: {e}")
...
def run_ocr():
    ...
    image.save(tmp_path)
    threading.Thread(target=process_ocr, args=(tmp_path, image.filename, engine_name, mqtt_client), daemon=True).start()
